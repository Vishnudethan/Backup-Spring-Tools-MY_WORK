	...
	public ResponseEntity<UserModel> deleteUser(Integer userId) {
		try {
			...
			if(um.isPresent()) {
				...
				return new ResponseEntity<UserModel>(um.get(),HttpStatus.FOUND);
			}
		}
		catch(Exception e) {
			System.out.println(e);
		...
		return new ResponseEntity<UserModel>(new UserModel(userId,null,null,null),HttpStatus.NOT_FOUND) ;
	}
